...
//        $this->middleware('auth');
    ...
    public function getdatatable(){
        ...
        foreach ($service_price_list as $row) {
            ...
            $temp['id'] = $row->id;
            if ($row->media_type == '1'){
            $media = "<div> <img src='".$baseurl.'/'.ASSET."/upload/image/thumbnail/".$row->media_name."' style='height:100px;' /></div>";
            $mediatype = "Image";
            }
            else{
            $media = "<video controls='' width='150'><source src='".$baseurl.ASSET."/upload/video/".$row->media_name."' id='video_here'>Your browser does not support HTML5 video.</video>";    
            $mediatype = "Video";
            }
